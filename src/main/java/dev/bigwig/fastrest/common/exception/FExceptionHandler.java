...
public class FExceptionHandler {
  ...
  public FError handleHttpRequestMethodNotSupportedException(HttpRequestMethodNotSupportedException e) {
    return new FError()
  ...
  public FError handleGenericJDBCException(GenericJDBCException e) {
    return new FError().setStatus(HttpStatus.BAD_REQUEST)
      .setMessage("传入字段错误：" + e.getMessage());
  }

  @ExceptionHandler(JwtException.class)
  public FError handleJwtException(JwtException e) {
    return new FError().setStatus(HttpStatus.UNAUTHORIZED)
      .setMessage("token 错误或已过期");
  }
}
